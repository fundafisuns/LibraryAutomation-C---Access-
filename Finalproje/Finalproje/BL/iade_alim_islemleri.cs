...
namespace BL
{
    public class iade_alim_islemleri
    {
        ...
        public List<almaverme> iade_alim_yukle()
        {
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    ...
                    almaverme yukle = new almaverme { kitapid = int.Parse(dr["kitapid"].ToString()), ogrenciid = int.Parse(dr["ogrenciid"].ToString()),
                    emanettarihi = dr["emanettarihi"].ToString(), iadetarihi = dr["iadetarihi"].ToString(), iadeedilentarih = dr["iadeedilentarih"].ToString(),
        ...
        public int tablo_iade_duzenleme(int kitapid, int ogrenciid, string islemturu,string iadeedilentarih)
        {
            ...
            int Sonuc = dl1.EkleSilGuncelle("update almaverme set iadeedilentarih='" + iadeedilentarih +"', islemturu='" + islemturu +
        ...
        public List<almaverme> kayit_listeleme1(int numara)
        {
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    almaverme kaydet = new almaverme { kitapid = int.Parse(dr["kitapid"].ToString()), ogrenciid = int.Parse(dr["ogrenciid"].ToString()),
                    emanettarihi = dr["emanettarihi"].ToString(), iadetarihi = dr["iadetarihi"].ToString(), iadeedilentarih = dr["iadeedilentarih"].ToString(),
                    borc = dr["borc"].ToString(), islemturu = dr["islemturu"].ToString() };
        ...
        public int emanet_islemi(int kitapid, int ogrenciid, DateTime emanettarihi, DateTime iadetarihi, string islemturu)
        {
            ...
            int Sonuc = dl2.EkleSilGuncelle("insert into almaverme (kitapid,ogrenciid,emanettarihi,iadetarihi,islemturu) values (" + kitapid + "," + ogrenciid + ",'" 
                + emanettarihi + "','" + iadetarihi + "','" + islemturu + "')", System.Data.CommandType.Text);
            return Sonuc;
        }
        ...
        public List<almaverme> ogrenci_emanet_iade_listele(string deger)
        {
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    almaverme kayit_etme = new almaverme { kitapid = int.Parse(dr["kitapid"].ToString()), ogrenciid = int.Parse(dr["ogrenciid"].ToString()),
                        emanettarihi = dr["emanettarihi"].ToString(), iadetarihi = dr["iadetarihi"].ToString(), iadeedilentarih = dr["iadeedilentarih"].ToString(),
                        borc = dr["borc"].ToString(), islemturu = dr["islemturu"].ToString() };
        ...
        public List<almaverme> kayit_listeleme2(string deger, string yer)
        {
            //listelemek istediğimiz verileri veritabanından çekiyoruz.
            //bu sefer çekmek istediğimiz sütunu da sorguya gönderiyoruz. 
            OleDbDataReader dr = dl4.DRVeriCek("Select * from almaverme where kitapid in (Select kitapid from kitaplar where " + yer + " ='" + deger + "')", CommandType.Text);
            if (dr.HasRows)
            {
                //veritabanından çektiğimiz verileri liste olarak geri gönderiyoruz.
                List<almaverme> islem_kayit = new List<almaverme>();
