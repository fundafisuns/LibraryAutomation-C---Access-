    ...
    public class ogrenci_islem
    {
        ...
        public List<ogrenciler> ogrenci_goster()
        {
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    ogrenciler ogrnci = new ogrenciler { ogrenciid = int.Parse(dr["ogrenciid"].ToString()), isim = dr["isim"].ToString(),
                        numara = dr["numara"].ToString(), bolum = dr["bolum"].ToString(), sehir = dr["sehir"].ToString() };
        ...
        public int ogr_ekle(string isim, string numara, string bolum,string sehir)
        {
            //öğrenci eklemek için sorgumuzu fonksiyona değerler ile birlikte gönderiyoruz.
            int sonuc = dl3.EkleSilGuncelle("INSERT into ogrenciler (isim,numara,bolum,sehir) VALUES ('" + isim + "','" + numara + "','" 
                + bolum + "','" + sehir + "')", System.Data.CommandType.Text);
            return sonuc;
        }
        DAL.DAL dl4 = new DAL.DAL();
        public int ogr_guncelle(int ogrenciid, string isim, string numara, string bolum,string sehir)
        {
            //öğrenci güncellemek için sorgumuzu fonksiyona değerler ile birlikte gönderiyoruz.
            int sonuc = dl4.EkleSilGuncelle("UPDATE ogrenciler SET isim='" + isim + "', numara='" + numara + "', bolum='" + bolum + "', sehir='" + sehir + 
